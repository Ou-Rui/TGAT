...
MODEL_SAVE_PATH = f'./saved_models/{args.prefix}-{args.agg_method}' \
                  f'-{args.attn_mode}-{args.data}.pth'
get_checkpoint_path = \
    lambda epoch: f'./saved_checkpoints/{args.prefix}-{args.agg_method}' \
                  f'-{args.attn_mode}-{args.data}-{epoch}.pth'
...
num_total_unique_nodes = len(total_node_set)  # number of nodes == max_idx

"""
    随机mask节点，用于验证未见过节点的LP？
    采样范围：ts > val_time的边的两端节点
    采样总结点数量的10%
"""
# ts_l > val_time：返回ts_l中，值大于val_time对应的索引，有多个索引组成列表
mask_node_set = set(
    random.sample(set(src_l[ts_l > val_time]).union(set(dst_l[ts_l > val_time])),
                  int(0.1 * num_total_unique_nodes)))
mask_src_flag = g_df.u.map(lambda x: x in mask_node_set).values  # ndarray: [E]，对应边的src是否被mask
mask_dst_flag = g_df.i.map(lambda x: x in mask_node_set).values
none_node_flag = (1 - mask_src_flag) * (1 - mask_dst_flag)  # ndarray: [E]：对应边的两端是否都被mask

# 训练集的边：时间要求 + 没有被mask。ndarray: [E]
...
new_node_set = total_node_set - train_node_set  # 未见过的节点集合

# val/test: 按照时间顺序
valid_val_flag = (ts_l <= test_time) * (ts_l > val_time)
valid_test_flag = ts_l > test_time
# 是否是新节点的边, shape=bool[E]: 两端节点至少一个是"新节点"
is_new_node_edge = np.array([(a in new_node_set or b in new_node_set)
                             for a, b in zip(src_l, dst_l)])
# val和test中的new_node_edge, shape=bool[E]
...
train_ngh_finder = NeighborFinder(adj_list, uniform=UNIFORM)  # 训练集只包含部分图（Inductive）
...
np.random.shuffle(idx_list)     # no use
...
for epoch in range(NUM_EPOCH):
    ...
    for k in range(num_batch):
        ...
        size = len(src_l_cut)
        # Negative Sampling, src_l_fake no use
    ...
    val_acc, val_ap, val_f1, val_auc = \
        eval_one_epoch('val for old nodes', tgan, val_rand_sampler,
                       val_src_l, val_dst_l, val_ts_l, val_label_l)

    nn_val_acc, nn_val_ap, nn_val_f1, nn_val_auc = \
        eval_one_epoch('val for new nodes', tgan, val_rand_sampler,
                       nn_val_src_l, nn_val_dst_l, nn_val_ts_l, nn_val_label_l)

    logger.info('epoch: {}:'.format(epoch))
    logger.info('Epoch mean loss: {}'.format(np.mean(m_loss)))
    logger.info('train acc: {}, val acc: {}, new node val acc: {}'
                .format(np.mean(acc), val_acc, nn_val_acc))
    logger.info('train auc: {}, val auc: {}, new node val auc: {}'
                .format(np.mean(auc), val_auc, nn_val_auc))
    logger.info('train ap: {}, val ap: {}, new node val ap: {}'
                .format(np.mean(ap), val_ap, nn_val_ap))
    # logger.info('train f1: {}, val f1: {}, new node val f1: {}'.format(np.mean(f1), val_f1, nn_val_f1))

    if early_stopper.early_stop_check(val_ap):
        logger.info('No improvement over {} epochs, stop training'
                    .format(early_stopper.max_round))
...
test_acc, test_ap, test_f1, test_auc = \
    eval_one_epoch('test for old nodes', tgan, test_rand_sampler,
                   test_src_l, test_dst_l, test_ts_l, test_label_l)

nn_test_acc, nn_test_ap, nn_test_f1, nn_test_auc = \
    eval_one_epoch('test for new nodes', tgan, nn_test_rand_sampler,
                   nn_test_src_l, nn_test_dst_l, nn_test_ts_l, nn_test_label_l)

logger.info('Test statistics: Old nodes -- acc: {}, auc: {}, ap: {}'
            .format(test_acc, test_auc, test_ap))
logger.info('Test statistics: New nodes -- acc: {}, auc: {}, ap: {}'
            .format(nn_test_acc, nn_test_auc, nn_test_ap))
