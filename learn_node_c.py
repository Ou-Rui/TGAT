...
for i_run in range(args.n_run):
  ...
  ''' mask label '''
  mask_train_label_l = train_label_l.copy()
  if args.mask > 0.0:
    num_mask = math.ceil(len(train_label_l) * args.mask)
    mask_idx = np.random.choice(len(train_label_l), num_mask, replace=False)
  ...
  for epoch in tqdm(range(args.n_epoch)):
    ...
    for k in range(num_batch):
      ...
      if args.mask > 0.0:
        lr_prob = lr_prob[src_label > -1]
        src_label = src_label[src_label > -1]
